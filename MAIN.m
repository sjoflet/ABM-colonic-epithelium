...
mkdir("ParentFolder", folder) % directs to ParentFolder within curent directory
...
% ** Inputs **     
final_graph = 1 ; % figure number for final graph
...
% general
nsteps = 2000 ; % iterations per simulation
inter = 400 ; % interval to publish dish screenshot on Dish Graph Figure
...
if saveit == 1 
workspacetitle = "1. Results/" +folder+ "/" + WStitle  ;
end 
...
if saveit == 1  
    save(workspacetitle,"DISH", "BT1_TOTPOP","BAC2","BT3_TOTPOP","DenseMucusThickness","LooseMucusThickness","AllPathDead", "DEADPATH", "METABOLITES1","METABOLITES2")
    dishfilename = fullfile('1. Results', folder,figname) ;
...
subplot(3,2,2)
hold on 
plot(t, BAC2, 'DisplayName', linelabels, 'Color', [myPlotColors(plotcolornum)],"LineWidth",2)
title('b. BT2')
set(gca,'TitleHorizontalAlignment', 'left','FontSize',15) ; 
xlabel('time (min)')
ylabel('Concentration (units)')
...
%%
if saveit == 1 
    finalgraphsname  = fullfile('1. Results', folder ,'PopulationPlots.fig') ;
